    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            v2f vert(appdata_t v)
            {
                ...
                float phase = time + worldPos.x * _PhaseOffset;
                // Sway mask: smooth transition from bottom to top
                float swayMask = saturate( (v.texcoord.y));

                // Sway in Y and X - scaled up for UI
                float swayY = sin(phase) * _SwayAmount * swayMask * 100.0;
                float swayX = cos(phase) * _SwayX * swayMask * 100.0;
